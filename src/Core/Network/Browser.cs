...
internal sealed class Browser : IDisposable
{
    ...
    private readonly byte[] _browsePacket;
    ...
    {
        var packet = WarcraftPacketProcessor.GenerateGameCancelledPacket(gameId);
        lock (_socketLock)
        {
    ...
    private void QueryTimer_Elapsed(object sender, ElapsedEventArgs e)
    {
        ...
        var foundGames = CollectResponses();
    ...
    private List<GameInfo> CollectResponses()
    {
        ...
        lock (_socketLock)
        {
            ...
            while (_browseSocket.Poll(0, SelectMode.SelectRead))
            {
                ...
                var gameInfo = WarcraftPacketProcessor.ParseGameInfoPacket(packet);
                ...
                var announcePacket = WarcraftPacketProcessor.GenerateGameAnnouncePacket(gameInfo);
    ...
    private void CheckGameLost()
    {
        ...
        var interval = DateTime.Now - _lastFoundServer;
