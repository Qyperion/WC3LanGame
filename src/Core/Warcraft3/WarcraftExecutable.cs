...
public static class WarcraftExecutable
{
    ...
    {
        var resolved = NormalizeExecutablePath(preferredPath);
        if (resolved != null)
            return resolved;

        foreach (var valueName in GetRegistryValueNames(warcraftType))
        {
    ...
    {
        var executablePath = ResolveExecutablePath(preferredPath);
    ...
    {
        var processes = FindWarcraftProcesses(preferredPath);
        try
        {
    ...
    public static string StopWC3ProcessRunning(string preferredPath = null)
    {
        ...
        try
        {
            var wc3Process = processes.FirstOrDefault();
            if (wc3Process == null)
    ...
    private static string NormalizeExecutablePath(string candidatePath)
    {
        ...
        try
        {
            var path = Environment.ExpandEnvironmentVariables(candidatePath.Trim().Trim('"'));
    ...
    private static List<Process> FindWarcraftProcesses(string preferredPath)
    {
        ...
        foreach (var processName in CollectProcessNames(preferredPath))
        {
            try
            {
